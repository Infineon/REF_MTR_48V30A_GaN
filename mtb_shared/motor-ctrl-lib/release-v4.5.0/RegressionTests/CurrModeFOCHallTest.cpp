...
TEST_GROUP(CurrModeFOCHallTest)
{
    CSysSim* sys_sim;
    ...
    void setup()
    {
        ...
        params.ctrl.mode = Curr_Mode_FOC_Hall;
...
TEST(CurrModeFOCHallTest, ClosedLoopPositiveCurrntCmd)
{
    ...
    sm.vars.capture_channels[5] = &vars.th_r_hall.elec;
    sm.vars.capture_channels[6] = &vars.w_hall.elec;
    ...
    ANGLE_EQUAL(Initial_Angle.elec, sys_sim->m_pmsm.m_th_r.elec, DEG_TO_RAD(0.5f));
    ...
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, vars.th_r_hall.elec, DEG_TO_RAD(5.0f)); // verify hall loop is locked
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_w_r.elec, vars.w_hall.elec, 0.05f); // verify hall loop is locked
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_T, vars.T_est, 0.05f);
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_r.q, vars.la_qd_r_est.q, 0.075f * params.motor.lam); // verify feedforward flux estimation
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_r.d, vars.la_qd_r_est.d, 0.075f * params.motor.lam); // verify feedforward flux estimation
    ...
    DOUBLES_EQUAL(vars.i_qd_r_cmd.q, sys_sim->m_pmsm.m_i_qd_r.q, 5.0f); // check controller
    DOUBLES_EQUAL(vars.i_qd_r_cmd.d, sys_sim->m_pmsm.m_i_qd_r.d, 5.0f); // check controller
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, vars.th_r_hall.elec, DEG_TO_RAD(7.5f)); // verify hall loop is locked
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_w_r.elec, vars.w_hall.elec, 0.01f); // verify hall loop is locked
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_T, vars.T_est, 0.05f);
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_r.q, vars.la_qd_r_est.q, 0.075f * params.motor.lam); // verify feedforward flux estimation
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_r.d, vars.la_qd_r_est.d, 0.075f * params.motor.lam); // verify feedforward flux estimation
    ...
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, sm.vars.capture_vals[5], DEG_TO_RAD(5.0f)); // vars.th_r_hall.elec
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_w_r.elec, sm.vars.capture_vals[6], 0.05f); // vars.w_hall.elec
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_T, vars.T_est, 0.05f);
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_r.q, sm.vars.capture_vals[7], 0.075f * params.motor.lam); // vars.la_qd_r_est.q
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_r.d, sm.vars.capture_vals[8], 0.075f * params.motor.lam); // vars.la_qd_r_est.d
...
TEST(CurrModeFOCHallTest, ClosedLoopNegativeCurrntCmd)
{
