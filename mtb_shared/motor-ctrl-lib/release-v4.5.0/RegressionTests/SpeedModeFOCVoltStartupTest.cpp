...
TEST_GROUP(SpeedModeFOCVoltStartupTest)
{
    CSysSim* sys_sim;
...
TEST(SpeedModeFOCVoltStartupTest, ClosedLoopPositiveSpeedCmd)
{
    ...
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, vars.th_r_final.elec, DEG_TO_RAD(5.0f));		// Verifying observer locked
    ...
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_s.q, vars.la_qd_s_est.q, 0.05f);					// Verifying observer locked
    DOUBLES_EQUAL(sys_sim->m_pmsm.m_la_qd_s.d, vars.la_qd_s_est.d, 0.05f);					// Verifying observer locked
    ...
    ANGLE_EQUAL(Wrap2Pi(MECH_TO_ELEC(sys_sim->m_mech_load.m_th.mech, params.motor.P)), sys_sim->m_pmsm.m_th_r.elec, DEG_TO_RAD(2.5f));
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, vars.th_r_fb.elec, DEG_TO_RAD(0.5f));
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, vars.th_r_est.elec, DEG_TO_RAD(2.5f));
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_r.elec, vars.th_r_final.elec, DEG_TO_RAD(2.5f));
#if defined(CTRL_METHOD_SFO)
    ANGLE_EQUAL(sys_sim->m_pmsm.m_th_s.elec, vars.th_s_est.elec, DEG_TO_RAD(2.5f));
    ...
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_la_qd_r.q, vars.la_qd_r_est.q, 0.05f);
    DOUBLES_EQUAL_PERC(sys_sim->m_pmsm.m_la_qd_r.d, vars.la_qd_r_est.d, 0.05f);
    POLAR_t i_polar_r;
...
TEST(SpeedModeFOCVoltStartupTest, ClosedLoopNegativeSpeedCmd)
{
